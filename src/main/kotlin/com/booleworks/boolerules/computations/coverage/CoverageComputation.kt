...
internal object CoverageComputation :
    ...
    override fun computeForSlice(
        ...
        baseConstraints += request.additionalConstraints.mapNotNull { processConstraint(cf, it, model, info, status)?.first?.formula() }
        val constraintsToCover = computeConstraintsToCover(cf, baseConstraints, request, model, info, slice, status) ?: return CoverageInternalResult(slice, emptyList())
    ...
    private fun computeConstraintsToCover(
        ...
        val uncompilableConstraints = mutableListOf<String>()
        val statusConstraints = ComputationStatusBuilder("", "", ComputationVariant.SINGLE)
        for (constraint in request.constraintsToCover) {
            val formula = processConstraint(cf, constraint, model, info, statusConstraints)?.first?.formula()
            if (formula == null) {
                uncompilableConstraints.add(constraint);
            } else if (baseSolver.satCall().addFormulas(formula).sat() == Tristate.FALSE) {
        ...
        if (uncompilableConstraints.isNotEmpty()) {
            status.addWarning(
                "For slice $slice the following constraints could not be compiled or transpiled and are therefore not covered by the result: ${
                    uncompilableConstraints.joinToString()
                }"
            )
        }
