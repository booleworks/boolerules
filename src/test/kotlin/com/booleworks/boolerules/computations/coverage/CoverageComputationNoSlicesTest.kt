...
internal class CoverageComputationNoSlicesTest : TestWithConfig() {
    ...
    fun testComputeWithFeatures(tc: TestConfig) {
        setUp(tc)
        val buildable = listOf("qzb", "zhd", "pow", "lmp", "xvo", "wyl", "zqm", "atu", "vpf", "vti", "suz", "gao", "csq", "emc", "eop", "jlt", "dxg", "xpx", "bhy", "fdj")
    ...
    fun testComputeWithFeaturesAndAdditionalRestriction(tc: TestConfig) {
        setUp(tc)
        val constraints = listOf("qzb & zhd", "-qzb / vti", "pow / lmp & -xvo & -bhy", "wyl & -zqm & emc", "-atu & xpx & bhy / oda")
    ...
    fun testComputeResponse(tc: TestConfig) {
        ...
        assertThat(response.detailMap[1]!![0].detail.configurations).hasSize(3).allMatch { it.coveredConstraints.isNotEmpty() }
    }
}
