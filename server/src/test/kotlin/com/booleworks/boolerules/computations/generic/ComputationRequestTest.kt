...
class ComputationRequestTest {
    ...
    fun testAddAllSlicingPropertiesForMissingFilters() {
        ...
        assertThat(request.sliceSelection).containsExactlyInAnyOrder(
            ...
            PropertySelectionDO(
                "model",
                PropertyTypeDO.ENUM,
                PropertyRangeDO(enumValues = setOf("M1", "M2", "M3", "M10"))
            ),
            PropertySelectionDO(
                "validity",
                PropertyTypeDO.DATE,
                PropertyRangeDO(dateMin = LocalDate.of(2022, 1, 1), dateMax = LocalDate.of(2026, 1, 12))
            ),
        ...
        val request = ConsistencyRequest(
            "ruleFile", mutableListOf(
                ...
                PropertySelectionDO(
                    "validity",
                    PropertyTypeDO.DATE,
                    PropertyRangeDO(dateMin = LocalDate.of(2022, 1, 1), dateMax = LocalDate.of(2023, 1, 12))
                ),
            ), listOf()
        )
        request.validateAndAugmentSliceSelection(model, SliceTypeDO.entries.toSet())
        ...
        val request = ConsistencyRequest(
            "ruleFile", mutableListOf(
                ...
                PropertySelectionDO(
                    "validity",
                    PropertyTypeDO.DATE,
                    PropertyRangeDO(dateMin = LocalDate.of(2022, 1, 1))
                ),
                PropertySelectionDO(
                    "release",
                    PropertyTypeDO.ENUM,
                    PropertyRangeDO(enumValues = setOf("R1", "R2", "R3"))
                ),
            ), listOf()
        )
        request.validateAndAugmentSliceSelection(model, SliceTypeDO.entries.toSet())
    ...
    fun testAddMissingLowerBoundsPropertiesForMissingFilters() {
        val request = ConsistencyRequest(
            "ruleFile", mutableListOf(
                ...
                PropertySelectionDO(
                    "validity",
                    PropertyTypeDO.DATE,
                    PropertyRangeDO(dateMax = LocalDate.of(2022, 1, 1))
                ),
                PropertySelectionDO(
                    "release",
                    PropertyTypeDO.ENUM,
        ...
        assertThat(request.sliceSelection).containsExactlyInAnyOrder(
            ...
            PropertySelectionDO(
                "validity",
                PropertyTypeDO.DATE,
                PropertyRangeDO(dateMin = LocalDate.of(2022, 1, 1), dateMax = LocalDate.of(2022, 1, 1))
            ),
            PropertySelectionDO("release", PropertyTypeDO.ENUM, PropertyRangeDO(enumValues = setOf("R1", "R2", "R3"))),
        )
    }
    ...
    fun testAddMissingPropertiesForMissingFilters() {
        ...
        assertThat(request.sliceSelection).containsExactlyInAnyOrder(
            PropertySelectionDO(
                "active",
                PropertyTypeDO.BOOLEAN,
                PropertyRangeDO(booleanValues = setOf(true, false)),
                SliceTypeDO.SPLIT
            ),
            PropertySelectionDO(
                "version",
                PropertyTypeDO.INT,
                PropertyRangeDO(intMin = 1, intMax = 5),
                SliceTypeDO.ANY
            ),
            PropertySelectionDO(
                "model",
        ...
        val request = ConsistencyRequest(
            "ruleFile", mutableListOf(
                PropertySelectionDO(
                    "version",
                    PropertyTypeDO.INT,
                    PropertyRangeDO(intValues = setOf(1, 2), intMin = 3),
                    SliceTypeDO.ANY
                ),
            ), listOf()
        )
        ...
        val request = ConsistencyRequest(
            "ruleFile", mutableListOf(
                PropertySelectionDO(
                    ...
                    PropertyRangeDO(
                        dateValues = setOf(LocalDate.now()),
                        dateMin = LocalDate.of(2022, 1, 1),
                        dateMax = LocalDate.of(2026, 1, 12)
                    )
                ),
            ), listOf()
        )
    ...
    fun testNotAllowedSliceTypeDefault() {
        ...
        assertThatThrownBy {
            request.validateAndAugmentSliceSelection(
                model,
                setOf(SliceTypeDO.SPLIT, SliceTypeDO.ALL)
            )
        }
            .isInstanceOf(IllegalStateException::class.java)
    ...
    fun testNotAllowedSliceType() {
        ...
        assertThatThrownBy {
            request.validateAndAugmentSliceSelection(
                model,
                setOf(SliceTypeDO.SPLIT, SliceTypeDO.ANY)
            )
        }
            .isInstanceOf(IllegalStateException::class.java)
